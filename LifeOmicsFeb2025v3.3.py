...
sample.to_csv("Temp.txt", sep='\t', index=False)
...
sample2.to_csv('Sample2Temp.txt', sep='\t', index=False)

#Take samples for case lists
...
#other case lists should be made for treatment and RNAseq data.
#write case_all files:
data = f"""cancer_study_identifier: Welm_V
stable_id: Welm_V_
...
stable_id: Welm_V_sequenced
case_list_name: Samples with mutation data
case_list_description: Samples with mutation data ({cases_cna}).
...
stable_id: Welm_V_sequenced
case_list_name: Samples with mutation data
case_list_description: Samples with mutation data ({cases_cnaseq}).
...
#sample = sample[['PATIENT_ID', 'SAMPLE_ID', 'SAMPLE_CLASS', 'PRIMARY_OR_RECURRENCE', 'WGS_ID', 'WGS_PT_NORMAL_ID', 'WES_ID', 'WES_PT_NORMAL_ID', 'RNASEQ', 'RNASEQ_META', 'RNASEQ_LIBRARY', 'CNV_(SEGFILESAMPLEIDS)', 'CNV_PT_NORMAL_(SEGFILESAMPLEIDS)2', 'CNV_PLATFORM/CHIP', 'AGE_AT_TISSUE_COLLECTION', 'ALT_ID', 'TOWARDS_ID', 'CC', 'CORE_SAMPLE_ID', 'COLLECTION_SITE', 'DISEASE', 'ER_PATIENT', 'PR_PATIENT', 'HER2_PATIENT', 'ER_PDX', 'PR_PDX', 'HER2_PDX', 'HCI_ID', 'INJECTION_SIDE', 'ORGANOID_PREP', 'DNA_EXTRACTION_', 'MOUSE_PASSAGE', 'ORGANOID_PASSAGE', 'CNV_PASSAGE', 'PASSAGE_DAY', 'CNV_DAY', 'SAMPLE_TYPE', 'SHADOW_ID', 'CORE_PATIENT_ID', 'TX_NAIVE_OR_PRETREATED', 'STUDY', 'EXPERIMENT', 'PDX_GROW_NOGROW', 'DATE_OF_COLLECTION_(ONLY_SAMPLES_W-O_SHADOWID)']]
#sample2 = sample[['PATIENT_ID', 'SAMPLE_ID', 'SAMPLE_CLASS', 'PRIMARY_OR_RECURRENCE', 'WGS_ID', 'WGS_PT_NORMAL_ID', 'WES_ID', 'WES_PT_NORMAL_ID', 'RNASEQ', 'RNASEQ_META', 'RNASEQ_LIBRARY', 'CNV_(SEGFILESAMPLEIDS)', 'CNV_PT_NORMAL_(SEGFILESAMPLEIDS)2', 'CNV_PLATFORM/CHIP', 'AGE_AT_TISSUE_COLLECTION', 'ALT_ID', 'TOWARDS_ID', 'CC', 'CORE_SAMPLE_ID', 'COLLECTION_SITE', 'DISEASE', 'ER_PATIENT', 'PR_PATIENT', 'HER2_PATIENT', 'ER_PDX', 'PR_PDX', 'HER2_PDX', 'HCI_ID', 'INJECTION_SIDE', 'ORGANOID_PREP', 'DNA_EXTRACTION_', 'MOUSE_PASSAGE', 'ORGANOID_PASSAGE', 'CNV_PASSAGE', 'PASSAGE_DAY', 'CNV_DAY', 'SAMPLE_TYPE', 'SHADOW_ID', 'CORE_PATIENT_ID', 'TX_NAIVE_OR_PRETREATED', 'STUDY', 'EXPERIMENT', 'PDX_GROW_NOGROW']]
...
df3.insert(1, column='START_DATE', value='')
df3.insert(2, column='STOP_DATE', value='')
df3.insert(3, column='EVENT_TYPE', value='TREATMENT')
df3.insert(4, column='TREATMENT_TYPE', value='MEDICATION')
df3.drop(['HCI PERSON ID'], axis = 1, inplace=True)
df3.drop(['AGE AT START'], axis = 1, inplace=True)
df3.drop(['AGE AT END'], axis = 1, inplace=True)
df3['START DATE'] = df3['START DATE'].astype(str)
df3['START DATE'] = df3['START DATE'].astype(str)
df3['START DATE'] = df3['START DATE'].str.split(' ',expand=True)[0]
df3['END DATE'] = df3['END DATE'].str.split(' ',expand=True)[0]
df3['START DATE'] = pd.to_datetime(df3['START DATE'])
df3['END DATE'] = pd.to_datetime(df3['END DATE'])
df3['START_DATE'] = df3['START DATE'] - df3['DATE OF BIRTH']
df3['START_DATE'] = df3['START_DATE'].astype(str)
df3['START_DATE'] = df3['START_DATE'].str.replace(' days', '')
df3['STOP_DATE'] = df3['END DATE'] - df3['DATE OF BIRTH']
df3['STOP_DATE'] = df3['STOP_DATE'].astype(str)
df3['STOP_DATE'] = df3['STOP_DATE'].str.replace(' days', '')
df3['STOP_DATE'] = df3['STOP_DATE'].str.replace('NaT', '')
df3.drop(['END DATE'], axis = 1, inplace=True)
df3.drop(['START DATE'], axis = 1, inplace=True)
df3.drop(['DATE OF BIRTH'], axis = 1, inplace=True)
df3.drop(['CURRENT?'], axis = 1, inplace=True)
df3.drop(['MEDICATION CLASS'], axis = 1, inplace=True)
df3.drop(['DOSE MIN'], axis = 1, inplace=True)
df3.drop(['DOSE MAX'], axis = 1, inplace=True)
df3.drop(['DOSE UNIT'], axis = 1, inplace=True)
df3.rename(columns={'MEDICATION': 'SUBTYPE_AGENT'}, inplace=True)
df3.rename(columns={'GPI CODE': 'CPT CODE'}, inplace=True)
df3.drop_duplicates(subset=['PATIENT_ID', 'START_DATE', 'STOP_DATE', 'EVENT_TYPE', 'TREATMENT_TYPE', 'SUBTYPE_AGENT'], inplace=True)


##Therapy total
df3_tx = pd.concat([df3a, df3b, chemo, rad, immuno, hormone, SYSTEMIC], ignore_index=True)
...
#Found excess duplicates where one had stop_date and the duplicate netry did not. 
...
#DIAGNOSTICS.rename(columns={'SUBTYPE_AGENT': 'AGENT'}, inplace=True)
