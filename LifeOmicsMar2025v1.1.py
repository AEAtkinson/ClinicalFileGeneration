...
def generate_meta_files(cancer_study_identifier, str_currentDateTime):
    ...
    for file_name in files:
        # Check if the file is a text file and not the PATIENT_ONCOPRINT_LEGEND file
        if file_name.startswith(str_currentDateTime) and "PATIENT_ONCOPRINT_LEGEND" not in file_name:
...
# Function to generate the meta-PATIENT_ONCOPRINT_LEGEND.txt file
...
# Generate the meta file for generic assay
generate_meta_generic_assay(args.study, args.cancer, str_currentDateTime)
...
shutil.copy(excel_file, os.path.join(directory_name, os.path.basename(excel_file)))
print(f"Moved {excel_file} to {directory_name}")

# Copy the script itself into the directory with the current date appended to the script name
script_name = os.path.basename(__file__)
script_name_with_date = f"{os.path.splitext(script_name)[0]}_{str_currentDateTime}{os.path.splitext(script_name)[1]}"
shutil.copy(script_name, os.path.join(directory_name, script_name_with_date))
print(f"Copied {script_name} to {directory_name} as {script_name_with_date}")

# Create a simple dated log file
log_file_name = f"log_{str_currentDateTime}.txt"
with open(log_file_name, 'w') as log_file:
    log_file.write(f"Log file created on {str_currentDateTime}\n")
    log_file.write(f"Meta files generated and moved to {directory_name} successfully.\n")
    log_file.write(f"case files generated and moved to {directory_name} successfully.\n")
    log_file.write(f"Cancer study identifier: {args.study}\n")
    log_file.write(f"Type of cancer: {args.cancer}\n")
    log_file.write(f"Input files:\n")
    log_file.write(f" - {excel_file}\n")

# Move the log file into the directory
shutil.move(log_file_name, os.path.join(directory_name, log_file_name))
print(f"Log file {log_file_name} moved to {directory_name}")
